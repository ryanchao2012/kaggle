...
if(!exist('dc_filter_test', 'var'))   % dc_filter_test
	load 'new_join/dc_filter_test.join'; 
end
...
for i = 1 : dev_num
	...
	ck_cand = dc_filter_test{dev_idx};
	...
	dev_bsc = double(basic_device_test(dev_idx, [4 7 8 9]));
	ck_bsc = [c1_score(:) c2_score(:) double(basic_cookie_test(ck_idx, [4 7 8 9]))];


	[ip_feature] = test_set_filter(ip_dev_ref, ip_ck_ref, dev_bsc, ck_bsc, ...

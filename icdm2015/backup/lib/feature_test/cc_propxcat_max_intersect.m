...
temp_weight(find(temp_weight < 4)) = 0;
...
for j = 1 : test_num
	...
	prop_idf = -1 * ones(n, 2);
	for i = 1 : n
		feature_dominant_ck1 = max(prop_feature_cookie{sample(i, 3)});
		feature_dominant_ck2 = max(prop_feature_cookie{sample(i, 4)});
		
		% prop_ck1 = prop_member_cookie{sample(i, 3)};
		% prop_ck2 = prop_member_cookie{sample(i, 4)};


		if(!isempty(feature_dominant_ck1) && !isempty(feature_dominant_ck2))
			dominant_idx = find(prop_feature_cookie{sample(i, 3)} == feature_dominant_ck1);
			prop_ck1 = prop_member_cookie{sample(i, 3)}(dominant_idx);

			dominant_idx = find(prop_feature_cookie{sample(i, 4)} == feature_dominant_ck2);
			prop_ck2 = prop_member_cookie{sample(i, 4)}(dominant_idx);
			...
			for k = 1 : mpck1
				...
				for kk = 1 : mpck2
					...
					if(!isempty(inter))
						...
						% total = [total length(inter)/((length(cat_ck1) * length(cat_ck2)))];
						[dummy idx_cat] = ismember(inter, category);
						total = [total sum(temp_weight(idx_cat))/((length(cat_ck1) * length(cat_ck2)))];
					else
						total = [total 0];
					end
				end
